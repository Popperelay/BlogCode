...
int main() {
	...
	shader stencilCube_shader("CubeOutlineVS.glsl", "CubeOutlineFS.glsl");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//利用模板绘制箱子轮廓（稍微放大的箱子）
		stencilCube_shader.Use();
		glUniformMatrix4fv(glGetUniformLocation(stencilCube_shader.shaderProgram, "view"), 1, GL_FALSE, value_ptr(view));
		glUniformMatrix4fv(glGetUniformLocation(stencilCube_shader.shaderProgram, "projection"), 1, GL_FALSE, value_ptr(projection));
		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
		glStencilMask(0x00);
		glDisable(GL_DEPTH_TEST); //保证轮廓不被挡住
		model = mat4();
		model = translate(model, glm::vec3(-1.0f, 0.0f, -1.0f));
		model = scale(model, vec3(1.1f, 1.1f, 1.1f));
		glUniformMatrix4fv(glGetUniformLocation(stencilCube_shader.shaderProgram, "model"), 1, GL_FALSE, value_ptr(model));
		glBindVertexArray(cubeVAO);
		glBindTexture(GL_TEXTURE_2D, cubeTexture);
		glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, 0);
		glStencilMask(0xFF);
