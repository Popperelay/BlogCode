...
vec3 CalcDirLight(DirectionalLight dirLight){
	//环境光
	vec3 ambientColor = dirLight.ambient * vec3(texture(material.diffuse_texture0,TexCoords));
	//vec3 ambientColor = vec3(texture(material.diffuse_texture0,TexCoords));
	
	//漫反射光
	vec3 normal = normalize(Normal);
	vec3 lightDir = normalize(-dirLight.direction);
	float diffuseFactor = max(dot(normal,lightDir),0.0f);
	vec3 diffuseColor = diffuseFactor * dirLight.diffuse * vec3(texture(material.diffuse_texture0,TexCoords));
	//镜面反射光
	...
	vec3 specularColor = specularFactor * dirLight.specular * vec3(texture(material.specular_texture0,TexCoords));
	return ambientColor + diffuseColor + specularColor;
}
//计算点光源
vec3 CalcPointLight(PointLight pointLight){
	//计算最终的环境光
	vec3 ambientColor = pointLight.ambient * vec3(texture(material.diffuse_texture0,TexCoords));  //环境光
	//计算最终的漫反射光
	...
	vec3 diffuseColor = diffuseFactor * pointLight.diffuse * vec3(texture(material.diffuse_texture0,TexCoords));   //漫反射光
	//计算最终的镜面反射光
	...
	vec3 specularColor = specularFactor * pointLight.specular * vec3(texture(material.specular_texture0,TexCoords)); //镜面反射光
...
vec3 CalcSpotLight(SpotLight spotLight){
	...
	//if(theta > spotLight.outCutOff){
		//环境光
		vec3 ambientColor = spotLight.ambient * vec3(texture(material.diffuse_texture0,TexCoords));
		//漫反射光
		vec3 normal = normalize(Normal);
		vec3 lightDir = normalize(spotLight.position - positionInWorld);
		float diffuseFactor = max(dot(normal,lightDir),0.0);
		vec3 diffuseColor = diffuseFactor * spotLight.diffuse * vec3(texture(material.diffuse_texture0,TexCoords));
		//镜面反射光
		vec3 reflectDir = normalize(reflect(-lightDir,normal));
		vec3 viewDir = normalize(viewPos - positionInWorld);
		float specularFactor = pow(max(dot(reflectDir,viewDir),0.0),material.shininess);
		vec3 specularColor = specularFactor * spotLight.specular * vec3(texture(material.specular_texture0,TexCoords));
		//聚光的衰减系数
	...
	//else{  //片元在聚光范围外时使用环境光，使其不至于完全黑暗
		//return spotLight.ambient * vec3(texture(material.diffuse_texture0,TexCoords));
