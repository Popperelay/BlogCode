...
int main() {
	...
	int FaceNum = FacePaths.size();
	GLint ImageWidth = 0, ImageHeight = 0, NumChannels = 0;
	for (int i = 0; i < FaceNum; ++i)
	{
		pImageData = stbi_load(FacePaths[i].c_str(), &ImageWidth, &ImageHeight, &NumChannels, 0);
		_ASSERT(pImageData);
	...
	CubeMap_Shader.Use();
	glUniformMatrix4fv(glGetUniformLocation(CubeMap_Shader.shaderProgram, "u_Projection"), 1, GL_FALSE, value_ptr(Projection));
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glUniformMatrix4fv(glGetUniformLocation(CubeMap_Shader.shaderProgram, "u_View"), 1, GL_FALSE, value_ptr(View));
		glBindTexture(GL_TEXTURE_2D, HdrTexture);
		renderCube();
